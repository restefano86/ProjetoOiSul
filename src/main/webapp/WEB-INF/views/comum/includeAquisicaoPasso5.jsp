<script src="/ProjetoOiSul/resources/js/bootstrap-filestyle.min.js" type="text/javascript"></script>

<img alt="aquisicao-passo-0" src="/ProjetoOiSul/resources/images/topAquisicaoPasso5.jpg" class="topAquisicao" >

<h2 class="tituloAquisicao">Envie o contrato assinado juntamente com a documentação necessária.</h2>

<div style="width: 600px; margin: 0 auto; text-align:center;">
	<input type="file" class="filestyle" data-buttonText="Selecionar Arquivo..." >
	<br>
	<input type="button" id="btGerarContratos" class="btn btn-primary" title="Enviar Arquivo" value="Enviar Arquivo" /> 
	<br>
	<br>Arquivos anexados ao pedido:<br>
	<a href="#">Pagina_1_assinada.jpg</a><br>
	<a href="#">Pagina_2_assinada.jpg</a><br>
	<a href="#">Pagina_3_assinada.jpg</a><br>
	<a href="#">Pagina_4_assinada.jpg</a><br>
</div>

<div id="navegacaoEtapasContratacao">
	<img alt="Etapa Anterior" src="/ProjetoOiSul/resources/images/btEtapaAnterior.jpg" 
		onclick="etapaAnterior();" class="btEtapaAnterior">
</div>

